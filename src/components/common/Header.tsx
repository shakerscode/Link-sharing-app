  ...
  return (
    <div className="bg-white rounded-2xl h-16 p-4 flex justify-between items-center">
      ...
      <div className="flex items-center gap-1">
        ...
        <h2 className="text-2xl font-bold text-gray-900">devlinks</h2>
      </div>
        ...
        {navLinks?.map((link: INav, i: number) => (
          <Link to={link?.path}>
            <button
              ...
              } px-4 hover:bg-violet-100 hover:text-violet-500 transition-all duration-300 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5`}
            >
              {link?.icon}
              <p>{link?.title}</p>
            </button>
